...
## Files
- `index.html` — entire UI + logic (state object + `render()`), using precompiled Tailwind in
  `styles.css` and the app's custom theme tokens.
...
# open http://localhost:8080/ihealthy/
```
(Open via a server, not file://, so shared assets and export behavior work consistently.)
...
- Share to LINE (same summary text builder), Share-as-Image (local html2canvas), Print/Save PDF (`window.print`), waiting-period section.
